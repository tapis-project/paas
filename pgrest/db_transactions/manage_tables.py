...
import psycopg2
from . import config
from .data_utils import do_transaction
...
def get_enums(db_instance=None):
    ...
    try:
        ...
        logger.info(f"Successfully retrieved enum data from postgres db.")
    except Exception as e:
        msg = f"Error retrieving enums: {e}"
        logger.error(msg)
        raise Exception(msg)

    # Parse enum object in a special way not using (parse_object_data())
    # enum formatted as {'tenant': {'enum1': ['enumval1', enumval2'],
    #                               'enum2': ['enumval1']},
    #                               ...}
    enum_dict = {}
    # rows come back in tenant, enum_name, enum_value order.
    for row in obj_unparsed_data:
    ...
    logger.info(f"Successfully got and parsed all enums in postgres_db. {enum_dict}")
    return enum_dict